...
function identifyInputType(inputString) {
  ...
  const s3BucketKeyPattern = new RegExp(
    "^[a-z0-9.-]{3,63}\\/[a-zA-Z0-9-._~:?#[\\]@!$&'()*+,;%=]+$"
  ); // bucket name and key

  const s3BucketPathPattern = new RegExp(
    "^[a-z0-9.-]{3,63}\\/([a-zA-Z0-9-._~:?#[\\]@!$&'()*+,;%=]+\\/)+$"
  ); // bucket name and path ending with /

  const hydrosharePattern = new RegExp(
    "^https:\\/\\/www\\.hydroshare\\.org\\/resource\\/[a-f0-9]{32}\\/$"
