...
function identifyInputType(inputString) {
    ...
    if (urlPattern.test(inputString)) {
        ...
        return "Invalid inputLink";
    }
}

/**
 * Get the download instruction given a link for a given platform
 * @param {string} inputLink the link to the resource
...
export function generateDataAccessCode(inputLink, platform) {
    ...
    if (platform === 'python') {
        if (inputType === "URL") {
            ...
            const [bucketName, ...pathArray] = inputLink.split('/');
            ...
            responseMessage = "Invalid inputLink";
        }
    } else if (platform === 'iguide') {
        responseMessage = `download_to_notebook("${inputLink}")`;
    } else {
